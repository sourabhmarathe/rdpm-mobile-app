...
bool isSakaLeapYear(int year) {
  return isGregorianLeapYear(year + 78);
}
...
double IndianCivilToJDN(SakaStruct date) {
  ...
  start =
      GregorianToJDN(DateTime(gyear, 3, isGregorianLeapYear(gyear) ? 21 : 22));
  caitra = leap ? 31 : 30;
...
SakaStruct convertToSakaDate(DateTime dt) {
  ...
  int jdn = trunc(GregorianToJDN(dt));
  return JDNToIndianCivil(jdn.toDouble());
}
